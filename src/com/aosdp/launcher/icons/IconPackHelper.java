...
public class IconPackHelper {
    ...
    public boolean loadIconPack(String packageName) {
        if (TextUtils.isEmpty(packageName) || TextUtils.equals(packageName, "default")) {
            return false;
        }
    ...
    public static void pickIconPack(final Context context) {
        ...
        builder.setAdapter(adapter, (DialogInterface dialog, int position) -> {
            if (adapter.isCurrentIconPack(position)) {
                return;
            }
            String selectedPackage = adapter.getItem(position);
            Utilities.getPrefs(context).edit().putString("icon_pack", selectedPackage).apply();
    ...
    public int getResourceIdForActivityIcon(LauncherActivityInfo info) {
    ...
    private static class IconAdapter extends BaseAdapter {
        ...
        IconAdapter(Context ctx, Map<String, IconPackInfo> supportedPackages) {
            ...
            Collections.sort(mSupportedPackages, (IconPackInfo lhs, IconPackInfo rhs) -> {
            ...
            mSupportedPackages.add(0, new IconPackInfo(defaultLabel, icon, "default"));
            mCurrentIconPack = Utilities.getPrefs(ctx).getString("icon_pack", "default");
        }
